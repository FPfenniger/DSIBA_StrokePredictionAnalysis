...
```
Here, we observe that some variables don't get recognized correctly as. `bmi` is a numerical feature and `hypertension` and `heart_disease` ought to be categorical. We will address these issues in the next steps.
...
categorical_vars <- c("gender", "ever_married", "work_type_children", "work_type_Govt_job", "work_type_Never_worked", "work_type_Private", "work_type_Self-employed", "Residence_type", "smoking_status_formerly smoked", "smoking_status_never smoked", "smoking_status_smokes", "stroke", "hypertension", "heart_disease")
encoded_stroke_tb[categorical_vars] <- lapply(encoded_stroke_tb[categorical_vars], factor)
head(encoded_stroke_tb)
...
# Create the barplot 
table(preprocessed_stroke_tb$gender == 'Other')
ggplot(preprocessed_stroke_tb, aes(x = factor(gender == "Other"))) +
  geom_bar() + 
