...
No duplicated rows are present and only the features `bmi` and `smoking_status` have missing values. We will address them in the next step.
...
knitr::kable(head(cleaned_stroke_tb))
...
knitr::kable(head(preprocessed_stroke_tb))
