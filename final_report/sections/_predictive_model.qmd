...
for (index1 in 1:(length(cat_variables) - 1)) {
  ...
  for (index2 in (index1 + 1):length(cat_variables)) {
    ...
    # Display only if Cramér's V > 0.3 and p-value < 0.05
...
suppressWarnings({
  ...
  combined_subplots <- subplot(
    ...
    layout(
      title = list(
        text = "Low-Risk Age Group: Over 60 Year Old",
        font = list(size = 16)
      ),
...
-   `avg_glucose_level`: The histogram of this variable still exhibits a
    ...
    mg/dL). We can clearly see, that stroke occurrence is more common in the diabetic range. 
...
suppressWarnings({
  ...
  combined_subplots <- subplot(
    ...
    layout(
      title = list(
        text = "High-Risk Age Group: Over 60 Year Old",
        font = list(size = 16)
      ),
      font = list(size = 10)  # General font size for combined plot
    )
...
combined_plot <- ggplotly(p) %>%
  layout(
    title = list(text = "High-Risk Age Group: Over 60 Years Old", x = 0.5), # Title centered
    margin = list(t = 50)
  )
...
We don't discuss the metrics as the model proved useless. A deeper discussion of the predictors will be done for the other models and in the final analysis part.
...
-   Negative coefficients for `Work Type` and `Ever_Married`: Government Job,
    Self-Employment and Marriage indicate that individuals in these categories are less
    likely to experience a stroke and serve as protective factors.
