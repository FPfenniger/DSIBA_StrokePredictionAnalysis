...
library(tidyverse)
```
--> still inconsistencies in the data cleaning part:

- remove gender_male and create a pure gender variable
- convert stroke into a factor (general factor conversion)
- problems in the balanced dataset 
...
We divided our analysis into four different datasets: the original dataset, the balanced original dataset, the low-risk age group, and the moderate-risk age group. We will train a logistic regression model for each dataset, before evaluating them on the test sets in the next chapter and comparing them to the baseline model.
...
```{r}
# Load the dataset
unbalanced_original_data <- read.csv("../../data/datasets/encoded_stroke_tb.csv")

# Convert the target variable to a factor with valid level names
unbalanced_original_data$stroke <- factor(unbalanced_original_data$stroke, 
                                          levels = c(0, 1), 
                                          labels = c("No", "Yes"))
...
train_unbalanced <- train_unbalanced %>% select(-gender_Male)
```
