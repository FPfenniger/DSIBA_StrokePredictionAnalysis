...
cat_variables <- c("hypertension", 
                   "heart_disease", "ever_married", "work_type_Self.employed", 
                   "work_type_Private", "work_type_Govt_job", "work_type_children", 
                   "Residence_type", "smoking_status_smokes", 
                   "smoking_status_never.smoked", "smoking_status_formerly.smoked", 
                   "gender")

# Placeholder for variables with high Cramér's V
...
for (index1 in 1:(length(cat_variables) - 1)) {
  ...
  for (index2 in (index1 + 1):length(cat_variables)) {
    ...
    if (cramers_v > 0.3 && chi_square_result$p.value < 0.05) {
      ...
      # Add variables to the selected list if not already added
      selected_variables <- unique(c(selected_variables, var1, var2))
    }
  }
}

# Display variables with high Cramér's V
cat("\nVariables with High Cramér's V (> 0.3):\n")
print(selected_variables)

# Calculate VIF for the selected variables
# Ensure selected variables exist in the dataset
if (length(selected_variables) > 0) {
  formula <- as.formula(paste("stroke ~", paste(selected_variables, collapse = " + ")))
  vif_model <- lm(formula, data = encoded_stroke_tb)  # Linear model for VIF computation
  vif_values <- vif(vif_model)
  
  # Display VIF values
  cat("\nVIF Values for Selected Variables:\n")
  print(vif_values)
} else {
  cat("No variables with high Cramér's V were identified.\n")
}
