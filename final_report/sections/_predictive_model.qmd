...
test_predictions_class <- factor(test_predictions_class, levels = levels(test_unbalanced$stroke))

# Confusion Matrix
conf_matrix <- confusionMatrix(test_predictions_class, test_unbalanced$stroke, positive = "Yes")
...
# Optional: Display the confusion matrix for context
print(conf_matrix)


```
The model achieves a high overall accuracy of 95.18%, but this metric is misleading due to the severe class imbalance in the dataset. Specifically, the sensitivity for the positive class (`Yes`) is 0, indicating that the model fails to identify any instances of strokes, which is critical for this context. While the specificity is perfect at 1.0, meaning the model correctly classifies all negative cases (`No`), this comes at the expense of entirely ignoring the minority class. The absence of any positive predictions results in undefined precision for the `Yes` class and a low balanced accuracy of 0.5, reflecting the model's inability to handle the imbalance effectively. These results underscore the need to address the class imbalance, as the model is heavily biased toward the majority class.
