...
```{r}
factor_columns <- c("gender", "hypertension", "heart_disease", "ever_married", 
                    "residence_type", "smoking_statusformerly_smoked", 
                    "smoking_statussmokes", "work_typechildren", 
                    "work_type_govt_job", "work_type_self_employed")

test_unbalanced[factor_columns] <- lapply(test_unbalanced[factor_columns], function(col) {
  factor(col, levels = c("X0", "X1"))  # Replace with actual levels from training
})

lapply(test_unbalanced[, factor_columns], levels)  # Check levels in test set
# Replace `factor_columns` with the list of categorical variables.
