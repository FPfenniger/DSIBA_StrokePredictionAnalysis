...
In the next step, we encode the remaining categorical variables with
multiple categories. Here we always define one reference category:
...
Where: - z is the standardized value, - x is the original feature
value, - mu is the mean of the feature, - sigma is the standard
...
observations to the all observations in actual class. In our case and
general in healthcare research, recall should be prioritized as
...
positives and false negatives. The formula for F1 score ranges from 0 to
1 and is given by: $$
...
$$ - Precision/Recall Curve for stratified
...
model's significant bias toward the majority class. For the evaluation
on the test set we therefore expect exclusive prediction of "No" for the
stroke variable and metrics in the same range as the training set.
...
As previously stated, our baseline model didn't predict any strokes at
all, which is reflected in the condfusion matrix.
...
As we have a narrow age range in this strata, we examine whether
removing `age` increases the predictive power of our logistic model. We
hope particularly to enhance our understanding of the predictor's
relative importance.

We train a new model, which only differs in a removed `age` column:
...
# Validate Levels of Predictions and Actuals
if (!all(levels(test_predictions_class) == levels(test_high_risk_ablated$stroke))) {
  stop("Mismatch in factor levels between predictions and actuals!")
}
