...
`age` and `avg_glucose_level` have very different summary statistics. The median age in the balanced dataset increased by 14 years to 59 years, up from 45 years in the original dataset. The average glucose level is 93,51 mg/dL, with only slight increase from 91,89 mg/dL.
...
# List of categorical variables for the analysis
variables_to_summarize <- c("gender", "ever_married", "work_type", 
...
create_bar_chart <- function(data, variable) {
  ...
  p <- ggplot(filtered_data, aes_string(x = variable, fill = "as.factor(stroke)")) +
    geom_bar(position = "fill") +
    labs(fill = "STROKE", y = "Proportion", title = paste("Distribution of", variable, "by Stroke")) +
    theme_minimal() +
...
create_violin_plot <- function(data, variable, stroke_variable) {
  ...
  plot <- plot_ly(data, x = ~get(stroke_variable), y = ~get(variable), type = "violin",
                  ...
                  meanline = list(visible = TRUE)) %>%
    layout(
      title = paste("Violin Plot of", variable, "by Stroke (Individuals of 60 and over)"),
      xaxis = list(title = "Stroke"),
      yaxis = list(title = variable)
    )
...
# Generate individual plots for individuals of 60 and over
...
combined_plot <- subplot(
  ...
  layout(title = "Violin Plots of Numerical Variables by Stroke (Individuals Of 60 Years and Over)")
