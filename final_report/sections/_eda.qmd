...
# Function to create bar plots for categorical variables grouped by stroke
create_bar_plot <- function(data, x_variable, stroke_variable = "stroke") {
  # Filter out "Other" from gender if applicable
  if (x_variable == "gender") {
    data <- data %>% filter(gender != "Other")
  }
  
  # Generate ggplot bar chart with "fill" by stroke
  if (x_variable %in% c("hypertension", "heart_disease")) {
    # Custom x-axis labels for binary variables
    p <- ggplot(data, aes_string(x = x_variable, fill = stroke_variable)) +
      geom_bar(position = "fill") +
      labs(fill = "Stroke", y = "Ratio", x = x_variable) +
      scale_x_discrete(labels = c("0" = "No", "1" = "Yes")) +  # Use discrete scale for binary variables
      theme_minimal() +
  ...
  # Convert to interactive Plotly plot
  ggplotly(p, tooltip = c("x", "y", "fill")) %>%  # Add tooltips for better interactivity
    layout(showlegend = FALSE)  # Legend will be displayed only once in the combined plot
}

# Example usage
# Replace `preprocessed_stroke_tb` with your actual dataset
categorical_variables <- c("gender", "ever_married", "work_type", "Residence_type", "smoking_status", "hypertension", "heart_disease")
plots <- list()  # List to store the plots

# Generate plots for each categorical variable
for (i in seq_along(categorical_variables)) {
  plots[[i]] <- create_bar_plot(outlier_stroke_tb, categorical_variables[i])
}

# Combine all plots into a grid layout (adjust rows as needed)
combined_plots <- subplot(
  plots,
  nrows = ceiling(length(categorical_variables) / 2),  # Arrange 2 plots per row
  titleX = TRUE,
  titleY = TRUE
) %>%
  layout(
    title = "Categorical Variables Grouped by Stroke",  # Global title
    showlegend = TRUE,  # Unified legend
    legend = list(
      x = 1.02,         # Position the legend outside the grid
      y = 1,            # Align to the top
      orientation = "v" # Vertical legend
    ),
    height = 1000,      # Adjust grid height
    width = 1200        # Adjust grid width
  )
