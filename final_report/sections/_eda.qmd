...
# Function to create one stacked bar plot for a categorical variable
one_bar_plot <- function(data, x_var) {
  # Summarize data to count occurrences for each combination of `x_var` and `stroke`
  summarized_data <- data %>%
    group_by(!!sym(x_var), stroke) %>%
    summarize(count = n(), .groups = "drop") %>%
    mutate(stroke = factor(stroke, labels = c("No Stroke", "Stroke"))) # Convert stroke to factor
  ...
  plot_ly(
    ...
    layout(
      title = x_var,         # Title for each subplot
      xaxis = list(title = ""), # No individual x-axis titles
      yaxis = list(title = "Count"),
      showlegend = FALSE     # Disable individual legends
    )
}

# Example dataset (replace `preprocessed_stroke_tb` with your dataset)
categorical_variables <- c("gender", "ever_married", "work_type", "Residence_type", "smoking_status", "hypertension", "heart_disease")
data <- preprocessed_stroke_tb

# Generate one plot per categorical variable
plots <- lapply(categorical_variables, function(x_var) one_bar_plot(data, x_var))

# Arrange all plots in a grid layout using subplot
fig <- subplot(
  plots,
  nrows = ceiling(length(categorical_variables) / 2), # Arrange in 2 columns
  shareX = TRUE,
  shareY = TRUE,
  titleX = TRUE,
  titleY = TRUE
  ...
  layout(
    title = "Categorical Variables Grouped by Stroke",
    legend = list(
      orientation = "h",   # Horizontal legend
      x = 0.5,             # Centered
      y = -0.2,            # Below the plots
      xanchor = "center"
    ),
    width = 1000,          # Overall plot width
    height = 800           # Overall plot height
  )
