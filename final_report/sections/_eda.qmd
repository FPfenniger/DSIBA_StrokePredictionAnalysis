...
# Filter data and select variables
...
long_data <- filtered_data %>%
  pivot_longer(cols = all_of(names(var_categories)), names_to = "variable", values_to = "category")

# Compute ordering based on Stroke proportion
...
# Order categories within each variable by ascending stroke proportion
...
# Order variables by their average stroke proportion
...
# Create a combined variable:category label
long_data <- long_data %>%
  mutate(var_cat = paste(variable, category, sep = ": ")) %>%
  arrange(var_rank, cat_rank)

# Set factor levels for var_cat
long_data$var_cat <- factor(long_data$var_cat, levels = unique(long_data$var_cat))

# Create the plot
# Using aes_string ensures 'stroke' is treated as a column name string.
p <- ggplot(long_data, aes_string(x = "var_cat", fill = "stroke")) +
...
# Add variable label at the end of each bar
label_data <- long_data %>%
  distinct(var_cat, variable) %>%
  mutate(xpos = 1.02)

p <- p + geom_text(
  data = label_data,
  aes_string(x = "var_cat", y = "xpos", label = "variable"),
...
# Create the plot
p <- ggplot(long_data, aes(x = var_cat, fill = stroke)) +
  geom_bar(position = "fill") +
  labs(
    title = "Distribution Across Risk Factors",
    x = NULL,        # After flipping, categories appear along y-axis
    y = "Proportion",
    fill = "Stroke Status"
  ) +
  scale_fill_manual(values = c("No Stroke" = "grey50", "Stroke" = "#1f77b4")) +
...
# Extract variable for labeling
label_data <- long_data %>%
  distinct(var_cat, variable) %>%
  mutate(xpos = 1.02) # Position to the right of 100%

# Add variable name label at the end of each bar
p <- p + geom_text(
  data = label_data,
  aes(x = var_cat, y = xpos, label = variable),
  hjust = 0, size = 3, color = "black"
)
