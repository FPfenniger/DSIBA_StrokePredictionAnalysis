...
scaled_stroke_tb <-
  read.csv("../../data/datasets/scaled_stroke_tb.csv")
```
...
>>>>>>> 490c0048d1ac08aacbdeab71605a39d2b72d9198
...
under_60_data_tb <- stratified_stroke_tb[stratified_stroke_tb$age_class == "under_60", ]
...
# save
write.csv(under_60_data_tb, "../../data/datasets/under_60_data_tb.csv", row.names = FALSE)
```
...
over_60_data_tb <- stratified_stroke_tb[stratified_stroke_tb$age_class == "60_and_over", ]
...
# save
write.csv(over_60_data_tb, "../../data/datasets/over_60_data_tb.csv", row.names = FALSE)
```
...
over_60_data_tb <- stratified_stroke_tb %>%
  filter(age_class == "60_and_over") %>%
...
# Use upSample to balance the over 60 stratified dataset
balanced_over_60_stroke_tb <- upSample(
  x = over_60_data_tb[, names(outlier_stroke_tb) != "stroke"], # All columns except 'stroke'
  y = over_60_data_tb$stroke, # Target variable for upsampling
  yname = "stroke" # Column name for the target variable in the upsampled dataset
)

# Check the distribution to confirm balancing
table(over_60_data_tb$stroke)

# save
write.csv(balanced_over_60_stroke_tb, "../../data/datasets/balanced__over_60_stroke_tb.csv", row.names = FALSE)
...
# Use upSample to balance the under 60 stratified dataset
balanced_under_60_stroke_tb <- upSample(
  x = under_60_data_tb[, names(outlier_stroke_tb) != "stroke"], # All columns except 'stroke'
  y = under_60_data_tb$stroke, # Target variable for upsampling
  yname = "stroke" # Column name for the target variable in the upsampled dataset
)

# Check the distribution to confirm balancing
table(balanced_under_60_stroke_tb$stroke)

# save
write.csv(balanced_under_60_stroke_tb, "../../data/datasets/balanced_under_60_stroke_tb.csv", row.names = FALSE)
>>>>>>> 490c0048d1ac08aacbdeab71605a39d2b72d9198
