...
normalize <- function(data) {
  ...
  data[numeric_cols] <- lapply(data[numeric_cols], function(x) {
    if (max(x, na.rm = TRUE) == min(x, na.rm = TRUE)) {
      return(rep(0, length(x)))  # Return 0 or any constant value
    } else {
...
# Normalize under_60_data
normalized_under_60_data <- normalize(under_60_data)

# Normalize over_60_data
normalized_over_60_data <- normalize(over_60_data)

# Display the first few rows of the normalized datasets to confirm normalization
head(normalized_under_60_data)
head(normalized_over_60_data)
