...
### EDA: Univariate analysis: Stroke cases are more frequent at higher glucose levels and in the age range of 60 to 80 years.
...
###EDA: Univariate Analysis: There exist weak positive relationships between age and stroke, and avg_glucose_level and stroke. No relationship between bmi and stroke.
```{r, message = FALSE}
#| code-fold: true
#| code-summary: "Click to show code"
library(knitr)
...
Potential Predictors for Further Exploration
...
Smoking: Potential risk factor
```
	•	Highest Proportion: Former smokers.
	•	Possibly partly influenced by age (former smokers are often older).
	•	Current smokers have slightly higher stroke proportions than never smokers.
```

Hypertension: Potential risk factor
```
	•	Higher stroke proportions in individuals with hypertension compared to those without.
```

ever_married:
	•	Observation: Married individuals show higher stroke proportions than unmarried.
	•	Possible Cause: Age-related (older individuals are more likely married and at greater stroke risk).
	
work_type:
	•	Highest Proportions: Private sector employees and self-employed individuals.
	•	Other work types show lower stroke case proportions.

heart disease:
	•	Observation: Higher stroke proportions in individuals with heart disease.
...
Since age is a potential risk factor, and other variables such as ever_married, former smokers, are also associated with age, we will explore the relationships in a bivariate analysis
## Model evaluation
