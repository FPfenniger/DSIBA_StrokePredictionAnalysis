...
# Count occurrences of each level in the 'gender' variable
gender_summary <- stroke_tb %>%
  group_by(gender) %>%
  summarize(count = n()) %>%
  rename(Gender = gender, Count = count)
...
```{r}
#table of patients with hypertension and without hypertension
library(dplyr)

# Summarize counts with and without hypertension
hypertension_summary <- preprocessed_stroke_tb %>%
  group_by(hypertension) %>%
  summarize(count = n())
print(hypertension_summary)
```

`hypertension`: 1,182 patients had hypertension, while 3,929 did not. Hypertension is more prevalent among older individuals in the population. Hypertension seems to be a risk factor for strokes, as the proportion of stroke cases is higher among individuals with hypertension. This aligns with medical knowledge that hypertension is a significant risk factor for strokes. The distribution of stroke cases is higher among individuals with hypertension, suggesting that hypertension may be a key predictor of stroke occurrence in this dataset.

`stroke`: Among the 5,111 patients, 249 had experienced a stroke, while 4,861 had not. `gender`: 2,994 female patients, 2,115 male patients, and 1 patient identified as other. `ever_married`: 3,529 patients were married, while 1,582 were not. `work_type`: The majority of patients were employed in private jobs (2,441), followed by self-employed (1,024), government jobs (657), and children (687). Only 22 patients had never worked. `Residence_type`: 2,290 patients lived in urban areas, while 2,821 lived in rural areas. `smoking_status`: 1,383 patients never smoked, 789 were former smokers, and 747 were current smokers. `heart_disease`: 306 patients had a history of heart disease, while 4,805 did not.

ersetze encoded_stroke_tb durch preprocessed_stroke_tb oder sogar eigene ohne other. so stimmts nicht mit 0, 1 , male, female.

```{r}
ggplot(encoded_stroke_tb, aes(x = age, fill = as.factor(stroke))) +
  geom_histogram(position = "fill") +
  labs(fill = "STROKE", y = "Ratio") +
  facet_wrap(~gender_Female, labeller = as_labeller(c(`0` = "Male", `1` = "Female")))
```

We can observe that there are no recorded cases of strokes in males under the age of 40. In contrast, there are multiple instances of strokes in females younger than 40, inclduing cases at ages 30, 15, and even below one year old.

Based on descriptive statistics, age, hypertension, heart Disease, and average glucose level could be potential candidates for further exploration and modeling.
...
sankey_data <- preprocessed_stroke_tb %>%
  filter(stroke == 1) %>%  # Only include stroke cases
  count(stroke, hypertension)

# Define nodes, now only for Stroke and hypertension categories
nodes <- data.frame(name = c("Stroke", "No Hypertension", "Hypertension"))

# Define links based on the filtered data
links <- sankey_data %>%
  mutate(source = 0,                        # Map stroke == 1 -> "Stroke"
         target = ifelse(hypertension == 0, 1, 2),  # Map hypertension: 0 -> "No Hypertension", 1 -> "Hypertension"
         value = n) %>%
  select(source, target, value)

# Create the Sankey plot with only Stroke paths
