...
# Define the UI
ui <- fluidPage(
  titlePanel("Age, Average Glucose Level, and BMI Distribution"),
  sidebarLayout(
    sidebarPanel(
...
# Define the UI
ui <- fluidPage(
  titlePanel("Interactive Stroke Data Distribution"),
  sidebarLayout(
    sidebarPanel(
...
`hypertension`: 1,182 patients had hypertension, while 3,929 did not. The dataset comprises 2,994 female patients, 2,115 male patients, and 1 patient identified as other. Average glucose level is 106.15 mg/dL, with a standard deviation of 45.28 mg/dL, and a range of 55.12 to 271.74 mg/dL. `stroke`: Among the 5,111 patients, 249 had experienced a stroke, while 4,861 had not. `gender` `ever_married` `work_type` `Residence_type` `smoking_status` `heart_disease`
...
```{r}
library(ggbeeswarm)
ggplot(data = preprocessed_stroke_tb, aes(x = factor(stroke), y = age)) +
  geom_beeswarm() +
