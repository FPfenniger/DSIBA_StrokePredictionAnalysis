...
The majoriy of persons are not subject to strokes in the dataset. This is in line with the general population. However for our analysis, we can try to optimise our dataset for later predictions by balancing the proportions of no_stroke and strokes. By either down-sampling meaning reducing the number of people with no strokes, or up-sampling enhancing the proportion of people with strokes.

Since the dataset is only made up of 5110 individuals, I chose to up-sample the dataset.

```{r}
# Check imbalance ratio by upsampling
# Upsample the data to balance the 'stroke' column
# Ensure the stroke column is a factor
...
# count how many people had a stroke and the prop in the balanced dataset
balanced_stroke_tb %>%
  group_by(stroke) %>%
  summarize(count = n()) %>%
  mutate(proportion = round(count / sum(count), 2))
