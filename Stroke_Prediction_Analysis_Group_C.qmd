...
# Example dataset (use your actual dataset)
# Assuming encoded_stroke_tb is your dataset and "target" is the binary target variable
encoded_stroke_tb <- data.frame(
  target = sample(c(0, 1), 100, replace = TRUE),
  age = rnorm(100, 50, 10),
  bmi = rnorm(100, 25, 5),
  avg_glucose_level = rnorm(100, 100, 20)
)

# Define UI for the app
ui <- fluidPage(
  titlePanel("Violin Plot for Stroke Prediction Variables"),
  sidebarLayout(
    sidebarPanel(
...
server <- function(input, output) {
  output$violinPlot <- renderPlotly({
    # Generate the violin plot for the selected variable
    p <- ggplot(encoded_stroke_tb, aes_string(x = "factor(target)", y = input$variable)) +
      geom_violin(fill = "skyblue", alpha = 0.7) +
      labs(x = "Stroke Occurrence", y = input$variable, title = paste(input$variable, "by Stroke Occurrence")) +
      theme_minimal()
