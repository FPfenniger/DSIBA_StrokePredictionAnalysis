...
```{r}
# Generate summary of your data frame
summary_table <- summary(preprocessed_stroke_tb)
...
# Define the UI
ui <- fluidPage(
  titlePanel("Numerical variables"),
  sidebarLayout(
    sidebarPanel(
...
server <- function(input, output, session) {
  ...
  output$histogramPlot <- renderPlotly({
    ...
    # Generate the ggplot histogram with mean and SD lines
    p <- ggplot(data.frame(data), aes(x = data)) +
      geom_histogram(binwidth = input$binwidth, fill = "dark grey", color = "white") +
      labs(title = paste("Distribution of", x_label), x = x_label, y = "Frequency") +
...
#### 3.1.1 Averages and standard deviations

`age`: The dataset includes patients with ages ranging from 0 to 82 years, with an average age of 43.23 years and a standard deviation of 22.61 years, and with a peak around 70 years old.
