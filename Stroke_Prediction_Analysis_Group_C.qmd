...
The dataset contains 5,110 patient records with 11 variables describing the patients. Collected attributes include gender, age, history of hypertension, history of heart disease, marital status, occupation sector, recidence type, average glucose levels, body mass index (BMI), smoking status, and stroke occurrence.

The dataset comprises 5,110 patient records. 4861 individuals have experienced no stroke, while 249 individuals have had a stroke.

```{r}
# Summary of the target variable 'stroke'
table(preprocessed_stroke_tb$stroke)
```

The dataset contains 11 variables that can be categorized into two types: numerical and categorical.

`Numerical variables` include - age - average glucose level - and body mass index (bmi),

while `categorical variables` include - gender - history of hypertension - history of heart disease - marital status - occupation sector - residence type - smoking status - and stroke occurrence.

This distinction is important because numerical and categorical variables require different types of plots and charts to effectively present their descriptive statistics.
...
# Subset the data frame to include only the desired variables
selected_data <- preprocessed_stroke_tb[, c("age", "avg_glucose_level", "bmi")]

# Generate summary for the selected variables
summary_table <- summary(selected_data)

# Convert the summary to a data frame
summary_df <- as.data.frame.matrix(summary_table)

# Display the table with kable
kable(summary_df, row.names = TRUE, col.names = c( "Age", "Avg Glucose Level", "BMI" ))
```

Based on the table and histogram, we can observe the following characteristics of the numerical variables: - `age`: The dataset includes patients with ages ranging from 0.8 to 82 years, with an average age of 43.23 years and a standard deviation of 22.61 years, and with a peak around 70 years old. - `avg_glucose_level`: Right-skewed distribution with a mean of 106.15 and a standard deviation of 45.28. The average glucose levels range from 55.12 to 271.74 mg/dL. - `bmi`: Right-skewed distribution with a mean of 28.89 and a standard deviation of 7.85. The BMI values range from 10.3 to 97.6.
...
Based on the histogram, `age` and `avg_glucose_level` exhibit notable differences in the proportions of stroke cases across their value ranges, suggesting they may be important predictors of stroke risk. In contrast, `bmi` shows no significant differences in stroke proportions, indicating it may not be strong predictors of stroke risk in this dataset. We will later carry out correlation analysis to further explore the relationships between these variables and the target variable `stroke`.

#### 3.1.2 Categorical variables

lets check the modal values and counts for the categorical variables.

```{r}
# Summary of categorical variables
# List of variables to summarize
variables_to_summarize <- c("gender", "ever_married", "work_type", "Residence_type", "smoking_status", "hypertension", "heart_disease")

# Create a function to summarize counts for each selected variable
summarize_selected_vars <- function(data, var) {
  data %>%
    group_by(.data[[var]]) %>%
    summarize(Count = n()) %>%
    rename(Type = .data[[var]]) %>%
    mutate(Variable = var)  # Add the variable name to the summary
}

# Loop over the selected variables and store the results in a list of tibbles
summary_list <- lapply(variables_to_summarize, function(var) {
  summarize_selected_vars(preprocessed_stroke_tb, var)
})
...
Todo: exclude other from the barchart
...
Relevant differences in the proportions of stroke cases can be observed across the levels of the following categorical variables. - `smoking_status`: The proportion of stroke cases is higher among individuals who formerly smoked compared to those who never smoked or currently smoke. - `hypertension`: Patients with hypertension have a higher proportion of stroke cases compared to those without hypertension. - `ever_married`: The proportion of stroke cases is higher among married individuals compared to those who have never married. - `work_type`: The distribution of stroke cases varies across different work types, with private sector employees and self-employed individuals showing higher proportions of stroke cases compared to other work types.

Not relevant appear to be the following variables as there are no differences in the proportions of stroke cases across the levels: - `gender` - `Residence_type`

Based on descriptive statistics, `heart_disease`, `smoking_status`, `hypertension`, `ever_married`, and `work_type` could be potential candidates for further exploration and modeling among the categorical variables. These variables exhibit notable differences in the proportions of stroke cases across their levels, suggesting they may be important predictors of stroke risk. In contrast, `gender` and `Residence_type` show no significant differences in stroke proportions, indicating they may not be strong predictors of stroke risk in this dataset.
...
--\> we don't do it like that, we use the VIF to control for multicollinearity
