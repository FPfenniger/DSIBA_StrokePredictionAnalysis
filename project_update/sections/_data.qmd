...
Two bar plots captured our attention: the one for the feature `gender`=`Other` and the one for the `work_type`=`Never worked`. The former appears only once in the dataset, while the latter appears just 22 times out of a total of 5,110 observations. Due to their extreme underrepresentation, we decided to exclude these observations from our analysis as they add unnecessary complexity.
