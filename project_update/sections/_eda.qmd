...
preprocessed_stroke_tb <- read.csv("../../data/preprocessed_stroke_tb.csv")
...
The graphic above displays the distribution of the categorical variables in relation to the target variable `stroke`. The variables `gender`, `hypertension`, and `heart_disease` are binary, while the variables `ever_married`, `work_type`, `residence_type`, and `smoking_status` have multiple categories. The bar chart shows the proportion of patients with and without a stroke for each category of the selected variable.

### 3. Bivariate Analysis 
In the bivariate analysis we examine correlations between our features. We use a correlation matrix to visualize the relationships between the numerical variables and the target variable `stroke`.
